...
class MyUserManager(BaseUserManager):
    def create_user(self, email, firstname, user_choices, password=None):
        if not email:
            raise ValueError('Votre adresse email est obligatoire !')

        user = self.model(email=self.normalize_email(email), firstname=firstname, user_choices=user_choices)

        user.set_password(password)
        user.save(using=self._db)
        return user

    def create_superuser(self, email, firstname, user_choices, password=None):
        user = self.create_user(email, firstname=firstname, user_choices=user_choices, password=password)
        user.is_admin = True
        user.is_staff = True
...
class CustomUser(AbstractBaseUser):
    ...
    REQUIRED_FIELDS = ['firstname', 'user_choices']
